...
const useColorPicker = (value, onChange) => {
    ...
    const currentColorObj = indexedColors?.filter((c) => (0, utils_js_1.isUpperCase)(c.value))[0] || indexedColors[0];
    ...
    const handleGradient = (newColor, left = currentLeft) => {
        let remaining = colors?.filter((c) => !(0, utils_js_1.isUpperCase)(c.value));
        let newColors = [
    ...
    const setR = (newR) => {
        let newVal = (0, formatters_js_1.formatInputValues)(newR, 0, 255);
        handleChange(`rgba(${newVal}, ${g}, ${b}, ${a})`);
    };
    const setG = (newG) => {
        let newVal = (0, formatters_js_1.formatInputValues)(newG, 0, 255);
        handleChange(`rgba(${r}, ${newVal}, ${b}, ${a})`);
    };
    const setB = (newB) => {
        let newVal = (0, formatters_js_1.formatInputValues)(newB, 0, 255);
        handleChange(`rgba(${r}, ${g}, ${newVal}, ${a})`);
    };
    const setA = (newA) => {
        let newVal = (0, formatters_js_1.formatInputValues)(newA, 0, 100);
        handleChange(`rgba(${r}, ${g}, ${b}, ${newVal / 100})`);
    };
    const setHue = (newHue) => {
        let newVal = (0, formatters_js_1.formatInputValues)(newHue, 0, 360);
        let tinyNew = (0, tinycolor2_1.default)({ h: newVal, s: s, l: l });
        let { r, g, b } = tinyNew.toRgb();
        handleChange(`rgba(${r}, ${g}, ${b}, ${a})`);
    };
    const setSaturation = (newSat) => {
        let newVal = (0, formatters_js_1.formatInputValues)(newSat, 0, 100);
        let tinyNew = (0, tinycolor2_1.default)({ h: h, s: newVal / 100, l: l });
        let { r, g, b } = tinyNew.toRgb();
        handleChange(`rgba(${r}, ${g}, ${b}, ${a})`);
    };
    const setLightness = (newLight) => {
        let newVal = (0, formatters_js_1.formatInputValues)(newLight, 0, 100);
        let tinyNew = (0, tinycolor2_1.default)({ h: h, s: s, l: newVal / 100 });
        if (tinyNew?.isValid()) {
            let { r, g, b } = tinyNew.toRgb();
            handleChange(`rgba(${r}, ${g}, ${b}, ${a})`);
        }
    ...
    const valueToCmyk = () => {
        let { c, m, y, k } = (0, converters_js_1.rgb2cmyk)(r, g, b);
        return `cmyk(${c}, ${m}, ${y}, ${k})`;
    };
